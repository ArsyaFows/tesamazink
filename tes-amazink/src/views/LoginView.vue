<template>
  ...
  <div class="min-h-screen flex items-center justify-center px-4 py-8"
    ...
    <!-- Login Card -->
    <div class="w-full max-w-md rounded-2xl shadow-xl overflow-hidden transition-all duration-300 transform hover:shadow-2xl"
         :style="{ backgroundColor: 'var(--header-bg)', border: '1px solid var(--border-color)' }">

      <!-- Gradient Header -->
      <div class="p-6 text-center"
           :style="{ background: 'linear-gradient(to right, var(--primary-bg), var(--primary-hover))' }">
        <h2 class="text-2xl font-bold text-white">Admin Login</h2>
        <p class="mt-1 text-sm" style="color: var(--secondary-text)">Masuk untuk mengakses dashboard</p>
      </div>
      ...
      <div class="px-6 py-4 text-center"
        ...
        <p class="text-xs" :style="{ color: 'var(--secondary-text)' }">
          Demo: Gunakan <code class="px-1 rounded" :style="{ backgroundColor: 'var(--border-color)' }">username</code>
          dan <code class="px-1 rounded" :style="{ backgroundColor: 'var(--border-color)' }">password</code>
        </p>
      </div>
    </div>
  </div>
